...
public class FormFieldMasterService : IFormFieldMasterService
{
    ...
    public List<(FORM_FIELD_Master Master, List<IDictionary<string, object?>> FieldConfigRows)> GetFormMetaAggregates(TableSchemaQueryType type)
    {
        ...
        var result = new List<(FORM_FIELD_Master, List<IDictionary<string, object?>>)>();

        foreach (var master in masters)
        {
            var configs = _con.Query(
                "SELECT * FROM FORM_FIELD_CONFIG WHERE FORM_FIELD_Master_ID = @id AND SCHEMA_TYPE = @schemaType",
